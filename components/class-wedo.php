...
public function __construct($instance_id=0){
	...
	$this->method_title='ONE';
	$this->method_description=' Zákazníci si na mapě snadno a pohodlně vyberou nejbližší výdejní místo ONE POINT nebo ONE BOX.';
	$this->enabled = get_option('wc_doprava_doprava_wedo');
...
public function calculate_shipping($package=array()){
	$rate=array('id'=>$this->get_rate_id(),'label'=>$this->title,'cost'=>$this->get_option('wedo_zakladni-cena'),'package'=>$package,);
	$this->add_rate($rate);
}
